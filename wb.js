...
api.m.jd.com
[rewrite_local]
^https?://api\.m\.jd\.com/api\?functionId=happyDigHelp url script-request-body https://raw.githubusercontent.com/yuannain1112/code/main/wb.js
...
function GetBody() {
    if ($request && $request.url.indexOf("functionId=happyDigHelp") >= 0) {
        if (typeof $request.body !== 'undefined'){
            ...
            $.log(
                `[${$.name}] 助力Body✅: 成功, export wbbody='${wbBodyVal}'`
            );
            $.msg($.name, `获取助力Body: 成功🎉`, `export wbbody='${wbBodyVal}'\n#设置变量`);
        };
        $done();
    }
}
